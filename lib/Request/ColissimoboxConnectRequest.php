...
/**
 * Class ColissimoboxConnectRequest
 */
class ColissimoboxConnectRequest extends AbstractColissimoRequest
{
    const WS_TYPE = 'CURL';
    const WS_PATH = '/api-ewe/v1/rest/urlCboxExt';
    const WS_CONTENT_TYPE = 'application/json';

    /**
     * ColissimoCboxConnectRequest constructor.
    ...
    public function __construct(array $credentials)
    {
        ...
        $this->request['token'] = '';
        $this->request['tagInfoPartner'] = 'PRESTASHOP';
        $module = Module::getInstanceByName('colissimo');
        if ($module && isset($module->version)) {
            $psVersion = _PS_VERSION_;
            $this->request['CUSER_INFO_TEXT_3'] = 'PS'.$module->version.' PS'.$psVersion;
        }
    }
    ...
    {
        return ColissimoboxConnectResponse::buildFromResponse($responseHeader, $responseBody);
    }
}
