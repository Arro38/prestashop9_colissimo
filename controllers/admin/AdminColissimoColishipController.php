...
class AdminColissimoColishipController extends ModuleAdminController
{
    ...
    public function processExportCsv()
    {
        ...
        $dbQuery = new DbQuery();
        // @formatter:off
        $dbQuery->select('co.*, coa.code_porte1, coa.code_porte2')
                ->from('colissimo_order', 'co')
                ->leftJoin('orders', 'o', 'o.`id_order` = co.`id_order`')
                ->leftJoin('colissimo_address', 'coa', 'coa.`id_cart` = o.`id_cart`')
        ...
        foreach ($results as $result) {
            ...
            $lineExp = array(
                ...
                'codeporte1' => $result['code_porte1'],
                'codeporte2' => $result['code_porte2'],
