...
class Colissimo extends CarrierModule
{
    ...
    public function setDefaultConfiguration()
    {
        ...
        Configuration::updateValue('COLISSIMO_POSTAL_PARTNER_BE', 0);
        ...
        Configuration::updateValue('COLISSIMO_DDP_US_COST', 6);
    ...
    public function getTabs()
    {
        ...
        foreach ($languages as $language) {
            if ($language['iso_code'] == 'fr') {
                $tabNames['orders'][$language['locale']] = 'Colissimo';
                $tabNames['orders'][$language['iso_code']] = 'Colissimo';
    ...
    public function postProcessConfiguration()
    {
        ...
        } elseif (Tools::isSubmit('submitColissimoDeliveryDateConfig')) {
            $this->context->smarty->assign('active', 'fo');
            $this->postProcessDeliveryDateConfiguration();
    ...
    public function checkColissimoSignature($credentials)
    {
        ...
        $this->logger->info(sprintf('Request check signature'), ['request' => json_decode($request->getRequest(true), true)]);
        try {
     ...
     * @throws Exception
     */
    public function getEstimatedDeliveryDateInfo()
    {
        $this->logger->setChannel('ColissimoBoxServices');
        $credentials = ColissimoTools::getCredentials();
        $request = new ColissimoBoxServicesRequest($credentials);
        ...
        $client->setRequest($request);
        $siteDepotList = [];
        $statutTunnelCommande = false;
        try {
            /** @var ColissimoBoxServicesResponse $response */
            $response = $client->request();
            if (!empty($response->siteDepotList)) {
                $siteDepotList = $response->siteDepotList;
            }
            if (isset($response->statutTunnelCommande)) {
                $statutTunnelCommande = $response->statutTunnelCommande;
            }
        } catch (Exception $e) {
            $this->logger->error('Exception thrown: ' . $e->getMessage());
        }
        ...
        return [
            'siteDepotList' => $siteDepotList,
            'statutTunnelCommande' => $statutTunnelCommande,
        ];
    }
     ...
     * @return array|string
     */
    public function getColissimoDeliveryDate($params)
    {
        $this->logger->setChannel('ColissimoBoxServices');
        $depositDate = ColissimoTools::getDepositDate();
        if (empty($depositDate)) {
            return '';
        }
        $credentials = ColissimoTools::getCredentials();
        $request = new ColissimoDeliveryDateRequest($credentials);
        $request->setDepositDate($depositDate)
            ->setRegateDepart($params['regate_code'])
            ->setZipCodeDest($params['postal_code'])
            ->buildRequest();
        $client = new ColissimoClient();
        $client->setRequest($request);
        try {
            /** @var ColissimoDeliveryDateResponse $response */
            $response = $client->request();

            return [
                'delivery_date' => isset($response->response['deliveryDate']) ? $response->response['deliveryDate'] : '',
                'short_message' => isset($response->response['shortMessage']) ? $response->response['shortMessage'] : ''
            ];
        } catch (Exception $e) {
            $this->logger->error('Exception thrown: ' . $e->getMessage());
        }
    ...
    {
        $enableDDP = (int) Tools::getValue('COLISSIMO_ENABLE_DDP');
        $ddpCosts = [
            'COLISSIMO_DDP_COST' => 18,
            'COLISSIMO_DDP_GB_COST' => 10,
            'COLISSIMO_DDP_US_COST' => 6,
        ];
        if ($enableDDP) {
            $ddpDefault = Tools::getValue('COLISSIMO_DDP_COST');
            $ddpGb = Tools::getValue('COLISSIMO_DDP_GB_COST');
            $ddpUs = Tools::getValue('COLISSIMO_DDP_US_COST');
            if ($ddpDefault === '' || !Validate::isFloat($ddpDefault)|| floatval($ddpDefault) < 0) {
                $ddpDefault = $ddpCosts['COLISSIMO_DDP_COST'];
                $this->context->controller->errors[] = $this->l('Invalid default DDP cost.');
            }
            if ($ddpGb === '' || !Validate::isFloat($ddpGb)|| floatval($ddpGb) < 0) {
                $ddpGb = $ddpCosts['COLISSIMO_DDP_GB_COST'];
                $this->context->controller->errors[] = $this->l('Invalid GB DDP cost');
            }
            if ($ddpUs === '' || !Validate::isFloat($ddpUs) || floatval($ddpUs) < 0) {
                $ddpUs = $ddpCosts['COLISSIMO_DDP_US_COST'];
                $this->context->controller->errors[] = $this->l('Invalid US DDP cost.');
            }
            Configuration::updateValue('COLISSIMO_DDP_COST', $ddpDefault);
            Configuration::updateValue('COLISSIMO_DDP_GB_COST', $ddpGb);
            Configuration::updateValue('COLISSIMO_DDP_US_COST', $ddpUs);
        }
        $percent = Tools::getValue('COLISSIMO_WEIGHTING_VALUE_PERCENT');
        $amount = Tools::getValue('COLISSIMO_WEIGHTING_VALUE_AMOUNT');
        if ($percent !== '' && !Validate::isFloat($percent)) {
            $this->context->controller->errors[] = $this->l('Please enter a valid percentage value.');
        }
        if ($amount !== '' && !Validate::isFloat($amount)) {
            $this->context->controller->errors[] = $this->l('Please enter a valid amount value.');
        }
     ...
     * @return void
     */
    public function postProcessDeliveryDateConfiguration()
    {
        $data = [
            'cutoff_hours' => [
                'monday' => Tools::getValue('COLISSIMO_CUTOFF_MONDAY'),
                'tuesday' => Tools::getValue('COLISSIMO_CUTOFF_TUESDAY'),
                'wednesday' => Tools::getValue('COLISSIMO_CUTOFF_WEDNESDAY'),
                'thursday' => Tools::getValue('COLISSIMO_CUTOFF_THURSDAY'),
                'friday' => Tools::getValue('COLISSIMO_CUTOFF_FRIDAY'),
                'saturday' => Tools::getValue('COLISSIMO_CUTOFF_SATURDAY'),
            ],
            'regate_code' => Tools::getValue('COLISSIMO_REGATE_CODE'),
            'text' => Tools::getValue('COLISSIMO_DELIVERY_DATE_MESSAGE'),
            'text_color' => Tools::getValue('COLISSIMO_DELIVERY_DATE_COLOR'),
            'text_police' => Tools::getValue('COLISSIMO_DELIVERY_DATE_FONT'),
        ];

        Configuration::updateValue('COLISSIMO_DELIVERY_DATE_OPTIONS', json_encode($data));
        $this->context->controller->confirmations = $this->l('Configuration saved successfully.');
    }
    ...
    public function ajaxProcessConnectToColissimobox()
    {
        ...
        $client = new ColissimoClient();
        $client->setRequest($request);
        $this->logger->info(sprintf('Request Connect Colissimo Box :'), ['request' => json_decode($request->getRequest(true), true)]);
        try {
            /** @var ColissimoboxConnectResponse $response */
            $colissimoLinks = $client->request();
            $response = $colissimoLinks->response;
            switch ($urlCbox) {
                case 'services':
                    $redirectUrl = $response['urlParamServices'];
                    break;

                case 'contact':
                    $redirectUrl = $response['urlConnectedCbox'];
                    break;

                case 'labelsPackaging':
                    $redirectUrl = $response['urlEtiquettesEtEmballages'];
                    break;
    ...
    public function setColissimoHeader()
    {
        ...
        $this->context->smarty->assign([
            'data' => $data,
            'colissimo_current_page' => Tools::getValue('controller'),
            'colissimo_controller_map' => [
                'orders' => 'AdminColissimoOrders',
                'dashboard' => 'AdminColissimoDashboard',
                'coliship' => 'AdminColissimoColiship',
                'affranchissement' => 'AdminColissimoAffranchissement',
                'deposit_slip' => 'AdminColissimoDepositSlip',
                'customs_documents' => 'AdminColissimoCustomsDocuments',
                'assign_orders' => 'AdminColissimoAssignOrders',
                'configuration' => Tools::getValue('module_name') == $this->name || Tools::getValue('configure') == $this->name,
            ],
        ]);
    }
        ...
        if ($idService) {
            ...
            if (Configuration::get('COLISSIMO_ENABLE_DDP') && in_array($isoCountryCustomer, ColissimoTools::$isoDDP)) {
                if ($isoCountryCustomer == 'GB') {
                    $shippingCost += (float) Configuration::get("COLISSIMO_DDP_GB_COST");
                } elseif ($isoCountryCustomer == 'US') {
                    $shippingCost += (float) Configuration::get("COLISSIMO_DDP_US_COST");
                } else {
                    ...
                    $shippingCost += (float) Configuration::get('COLISSIMO_DDP_COST');
                }
            }
    ...
    public function getOrdersToTrack()
    {
        ...
        $dbQuery->select('cola.id_colissimo_label')
            ...
            ->where('cola.date_add > "' . pSQL($dateAdd->format('Y-m-d H:i:s')) . '" ' . Shop::addSqlRestriction(false, 'o'));
            ...
            if ($idService) {
                if (Validate::isLoadedObject($deliveryAddr)) {
                    ...
                    if (in_array($isoCountryCustomer, ColissimoTools::$isoDDP)) {
                        ...
                        if ($isoCountryCustomer == 'US') {
                            $colissimoDDP = Configuration::get('COLISSIMO_DDP_US_COST');
                        }
                        $this->context->smarty->assign(
                            [
                    ...
                    if ($device == 'mobile') {
                        $this->context->smarty->assign(
                            [
                                ...
                                'osm_relay_filter_mobile' => Configuration::get('COLISSIMO_OSM_RELAY_FILTER_MOBILE'),
                        ...
                        $this->context->smarty->assign(
                            [
                                ...
                                'osm_relay_filter' => Configuration::get('COLISSIMO_OSM_RELAY_FILTER'),
                            ]
                        );
                    }
                }
                    ...
                    if ($device == 'mobile') {
                        $this->context->smarty->assign(
                            [
                                ...
                                'widget_relay_filter_mobile' => Configuration::get('COLISSIMO_WIDGET_RELAY_FILTER_MOBILE'),
                            ]
                        );
                    } else {
                        $this->context->smarty->assign(
                            [
                                ...
                                'widget_relay_filter' => Configuration::get('COLISSIMO_WIDGET_RELAY_FILTER'),
                            ]
                        );
                    }
                }
        ...
        if (Validate::isLoadedObject($order) && $order->current_state != Configuration::get('PS_OS_ERROR')) {
            ...
            if (Configuration::get('COLISSIMO_ENABLE_DDP') && $carrier->is_free == 0) {
                if (in_array($isoCountryCustomer, ColissimoTools::$isoDDP)) {
                    ...
                    if ($isoCountryCustomer == 'US') {
                        $colissimoOrder->ddp_cost = Configuration::get("COLISSIMO_DDP_US_COST");
                    }
            ...
            // Disable DDP if the DDP cost equals 0
            if ($colissimoOrder->ddp_cost == 0) {
                $colissimoOrder->ddp = 0;
            }
            $colissimoOrder->hidden = 0;
            try {
    ...
    public function hookDisplayCarrierExtraContent($params)
    {
        ...
        if ($this->getEstimatedDeliveryDateInfo()['statutTunnelCommande']) {
            $cart = $params['cart'];
            $deliveryAddress = new Address((int) $cart->id_address_delivery);
            $countryIso = Country::getIsoById($deliveryAddress->id_country);
            if (strtoupper($countryIso) == 'FR') {
                $deliveryDateConfig = json_decode(Configuration::get('COLISSIMO_DELIVERY_DATE_OPTIONS'), true);
                $deliveryDateConfig['postal_code'] = $deliveryAddress->postcode;
                $response = $this->getColissimoDeliveryDate($deliveryDateConfig);
                if (!empty($response)) {
                    $text = isset($deliveryDateConfig['text']) ? trim($deliveryDateConfig['text']) : '';
                    $textConfigured = $text !== '';
                    $messageToDisplay = $textConfigured ? $text : (isset($response['short_message']) ? $response['short_message'] : '');
                    $estimatedDelivery = isset($response['delivery_date']) ? $response['delivery_date'] : '';
                    $this->context->smarty->assign([
                        'colissimo_estimated_delivery' => $estimatedDelivery,
                        'colissimo_delivery_message' => $messageToDisplay,
                        'colissimo_text_color' => isset($deliveryDateConfig['text_color']) ? $deliveryDateConfig['text_color'] : '',
                        'colissimo_font' => isset($deliveryDateConfig['text_police']) ? $deliveryDateConfig['text_police'] : '',
                        'colissimo_show_estimated_date' => $textConfigured,
                    ]);
                }
            }
        }
     ...
     * @throws PrestaShopException
     */
    public function hookActionGetOrderShipments($params)
    {
