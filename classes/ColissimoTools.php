...
class ColissimoTools
{
    ...
    /** @var array Countries (101 ISO) */
    public static $isoDDP = ['ZA', 'DZ', 'AO', 'AI', 'AG', 'SA', 'AR', 'AU', 'BH', 'BD', 'BZ', 'BT', 'BO', 'BA', 'BR', 'BN', 'KH', 'CM', 'CA', 'CV', 'CL', 'CN', 'CO', 'CG', 'KR', 'CR', 'CI', 'DJ', 'DM', 'EG', 'AE', 'EC', 'US', 'ET', 'FJ', 'GA', 'GH', 'GI', 'GY', 'HT', 'HN', 'HK', 'CK', 'IN', 'ID', 'IQ', 'IS', 'JP', 'JO', 'KE', 'KW', 'LA', 'LB', 'LR', 'LY', 'MG', 'MV', 'MA', 'MU', 'MR', 'MX', 'MN', 'MS', 'MZ', 'NP', 'NI', 'NE', 'NG', 'NO', 'NZ', 'UG', 'OM', 'UZ', 'PK', 'PA', 'PG', 'PY', 'PE', 'PH', 'QA', 'RW', 'GB', 'SV', 'WS', 'SN', 'SG', 'LK', 'SR', 'CH', 'TZ', 'TH', 'TL', 'TG', 'TO', 'TN', 'TR', 'UY', 'VU', 'VN', 'ZW'];

    /** @var array Countries that allow the choice of delivery either via the postal partner or via DPD */
    public static $isoPostalPartner = ['DE', 'AT', 'IT', 'LU', 'BE'];
     ...
     * @return bool
     */
    public static function isValidBlockingCodeAmount($minAmount, $maxAmount)
    {
    ...
    /**
     * Calculates the deposit date based on the configured cutoff time
     *
     * @return string
     */
    public static function getDepositDate()
    {
        ...
        if (!isset($deliveryDateConfig['cutoff_hours'])) {
            return '';
        }
        $cutoffHours = $deliveryDateConfig['cutoff_hours'];
        $now = new DateTime();
        $today = strtolower($now->format('l'));
        $cutoffToday = isset($cutoffHours[$today]) ? $cutoffHours[$today] : '15:00';
        $cutoffTime = DateTime::createFromFormat('Y-m-d H:i', $now->format('Y-m-d') . ' ' . $cutoffToday);
        // Set the deposit date to the next day
        if ($now > $cutoffTime) {
            $timestamp = $now->getTimestamp() + 86400;
            $depositDate = new DateTime();
            $depositDate->setTimestamp($timestamp);
            $nextDay = strtolower($depositDate->format('l'));
            if ($nextDay === 'sunday') {
                $timestamp += 86400;
                $depositDate->setTimestamp($timestamp);
            }

            return $depositDate->format('Y-m-d');
        }
        // else today
