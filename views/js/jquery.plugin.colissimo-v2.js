...
jQuery.extend(jQuery.fn, {
    frameColissimoOpen: function (params) {
        ...
        html.push('		<td class="widget_colissimo_table_adresse">');
        html.push('			<input id="widget_colissimo_adresse" type="text" class="widget_colissimo_input_text_normal" placeholder="23 rue de Rivoli, 75004 Paris, FR" ></input>');
        html.push('		</td>');
...
//		html = html + '	</tr>');
        ...
        var adr = strNoAccent(params.ceAddress).replace("’", "'") + ', ' + params.ceZipCode + ' ' + params.ceTown.replace("’", "'") + ', ' + params.ceCountry;
        colissimo("#widget_colissimo_adresse").val(params.ceAddress + ', ' + params.ceZipCode + ' ' + params.ceTown+ ', ' + params.ceCountry);
        ...
        if (adr == '') {
            ...
            colissimo.ajax({
                ...
                data: "data=" + window.btoa(JSON.stringify({addresse: adr, origin : params.origin, filterRelay : params.filterRelay, token: params.token})),
                success: function (msg) {
                    console.log(msg);
...
function colissimo_widget_validatePDR(i) {
    ...
    if (colissimo_widget_callBack != null) {
        window[colissimo_widget_callBack](colissimo_widget_listPoint[i-1]);
    }
    //on ferme le widget
    //conteneur_widget_colissimo.html('');
}
...
function widget_colissimo_reloadPDR() {
    ...
    if (tab != null && tab.length > 1) {
        ...
        // Match international :
        // - FR/BE/CH: 75001, BE-1000, CH-1211
        // - PT: 1234-567
        var match = line.match(/^([A-Z]{0,2}-?\d{4,5}(?:-\d{3})?)\s+(.+)$/i);

        if (match) {
            params.ceZipCode = match[1].trim();
            params.ceTown = match[2].trim();
        } else {
            // fallback : tout est mis comme ville
            params.ceTown = line;
        }
    }
    ...
    //On recharge la map
   // var adr = strNoAccent(params.ceAddress) + ', ' + params.ceZipCode + ' ' + params.ceTown + ', ' + params.ceCountry;
    var adr = strNoAccent(recherche);
    console.log('adr = ' + adr);
    colissimo.ajax({
        type: "GET",
        encoding: "UTF-8",
...
//	}
    colissimo.ajax({
        ...
        data: "data=" + window.btoa(JSON.stringify({
            addresse: strNoAccent(params.ceAddress).replace("’", "'"),
            codePostal: params.ceZipCode,
            ville: strNoAccent(params.ceTown).replace("’", "'"),
